...
class IbgeDados
  ...
  def self.localidades_all
    ...
    json.map do |dados|
    #puts dados[:municipio][:microrregiao][:mesorregiao][:UF][:nome]
    puts dados[:id],dados[:nome],dados[:municipio][:microrregiao][:mesorregiao][:UF] dados[:id], dados[:nome], dados[:sigla]
  end 
  end
